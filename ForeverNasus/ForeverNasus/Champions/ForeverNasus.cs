...
namespace ForeverAIO.Champions
{
    class Nasus : Champion
    {
        ...
        {
            Q = new Spell(SpellSlot.Q, xSLxOrbwalker.GetAutoAttackRange(Player));
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("Key", "Key");
            {
                ...
                key.AddItem(new MenuItem("LaneClearActive", "Lane Clear!", true).SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("LastHitKey", "Last Hit!", true).SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("SpellMenu", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Lane_Clear", "Q in Lane Clear", true).SetValue(true));
                    qMenu.AddItem(new MenuItem("Q_Last_Hit", "Q to Last Hit", true).SetValue(true));
                    spellMenu.AddSubMenu(qMenu);
                }

                var eMenu = new Menu("EMenu", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Lane_Clear", "E in Lane Clear", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Lane_Clear_Mana", "E Minimum Mana % in Lane Clear", true).SetValue(new Slider(50, 0, 100)));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("RMenu", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_If_HP", "R If HP% <=", true).SetValue(new Slider(30, 0, 100)));
            ...
            var miscMenu = new Menu("Misc", "Misc");
            {
                ...
                miscMenu.AddItem(new MenuItem("W_Interrupt", "Use W to interrupt", true).SetValue(true));
                //add to menu
